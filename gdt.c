...
extern void gdt_flush(uint32, uint32);
...
void init_gdt()
{
  ...
  uint32 gdt_kernel_segments_offset[NUM_OF_KERNEL_SEGMENTS];
  ...
  for (uint32 i = 0, j = 8; i < NUM_OF_KERNEL_SEGMENTS; i++)
  {
    gdt_kernel_segments_offset[i] = j;
    j += 8;
  }
  gdt_flush((uint32)&gdt_ptr, (uint32)&gdt_kernel_segments_offset);
}