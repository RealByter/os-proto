...
uint8 ide_print_error(uint32 drive, uint8 err)
{
  ...
  else if (err == 2)
  {
    ...
    if (st & ATA_ER_AMNF)
    {
      fb_write("- No Address Mark Found\n", 24);
      err = 7;
    }
    if (st & ATA_ER_TK0NF)
    {
      fb_write("- No Media or Media Error\n", 26);
      err = 3;
    }
    if (st & ATA_ER_ABRT)
    {
      fb_write("- Command Aborted\n", 18);
      err = 20;
    }
    if (st & ATA_ER_MCR)
    {
    ...
    if (st & ATA_ER_UNC)
    {
      fb_write("- Uncorrectable Data Error\n", 27);
      err = 22;
    }
    if (st & ATA_ER_BBK)
    {
      fb_write("- Bad Sectors\n", 14);
      err = 13;
    }
  }
  else if (err == 3)
  {
    fb_write("- Reads Nothing\n", 16);
    err = 23;
  }
  else if (err == 4)
  {
    fb_write("- Write Protected\n", 18);
    err = 8;
  }
